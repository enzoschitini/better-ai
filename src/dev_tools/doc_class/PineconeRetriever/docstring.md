...
class PineconeRetriever:
    ...
    Args: 
    :param client (Optional[PineconeClient]): Objeto previamente configurado contendo o índice,
        embeddings e namespace padrão. Default é None, o que cria um cliente padrão.

    Methods:
            similarity_search(query, k=5, filter_search=None): Executa uma busca por similaridade vetorial.
            get_all_docs_by_metadata(batch_size=None, dimension=None, target_key='file_id', target_value=None):
                Recupera vetores com base em filtro de metadados.
    """
    ...
    def similarity_search(
        ...
        Args: 
            ...
            k (int): Quantidade máxima de resultados a serem retornados. Default é 5.
            filter_search (Optional[Dict[str, Any]]): Filtro opcional no formato {campo: valor} ou {campo: [valores]}.

        Returns:
            List[Dict[str, Any]]: Lista de documentos similares contendo id, texto, metadados e score.

        Raises:
            ValueError: Se query for vazia ou k menor ou igual a zero.
            RuntimeError: Se ocorrer falha na geração do embedding, consulta ao Pinecone ou processamento dos resultados.
        """
        # ==========================
        # Validações
        # ==========================
    ...
    def get_all_docs_by_metadata(
        ...
        Args: 
            batch_size (int | None): Quantidade máxima de vetores retornados por requisição.
                Controla paginação, uso de memória e latência. Default é None, que usa o batch_size da configuração.
            dimension (int | None): Dimensão do vetor do índice (necessária para o dummy vector).
                Default é None, que usa a dimensão da configuração.
            target_key (str): Chave do metadata usada como filtro (ex: file_id, user_id). Default é "file_id".
            target_value (Union[str, List[str]]): Valor ou lista de valores usados no filtro.

        Returns:
            List[Dict[str, Any]]: Lista de vetores recuperados do Pinecone com id, metadados e score.

        Raises:
            ValueError: Se target_value for vazio.
            RuntimeError: Se ocorrer falha durante a recuperação paginada.
        """
        if not target_value:
            tracer.ERROR(
