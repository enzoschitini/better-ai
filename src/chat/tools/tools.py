...
def get_tools_config(selected_tools, tool_dic):
    ...
    def fraciona_salario(divisao: int):
        ...
        df = pd.read_csv(f"LangChain Chat/ChatModel/{tool_dic["fraciona_salario_dic"]["dataframe"]}.csv")
        dic = df[df["ID"] == tool_dic["fraciona_salario_dic"]["user_id"]].to_dict()
        cliente = {col: val[tool_dic["fraciona_salario_dic"]["value"]] for col, val in dic.items()}
